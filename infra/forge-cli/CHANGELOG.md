# @granite-js/forge-cli

## 2.5.4

### Patch Changes

- @granite-js/deployment-manager@2.5.4
- @granite-js/plugin-core@2.5.4

## 2.5.3

### Patch Changes

- 8b04ab6: Add configurable deployment channels across Forge, deployment storage and the AWS CDN. Forge automatically
  registers each app/channel selector in S3, so channel creation requires no per-channel infrastructure configuration.
  Channel objects, state, history and cluster pointers are isolated under `channels/<channel>/`. Registered path
  suffixes select channels while the legacy default and unregistered filename tags keep their existing routes.
  Conditional selector reservations and checks against retained legacy bundles prevent tag/channel name conflicts
  between updated publishers. Registration and rollout events invalidate service selectors, and unique invalidation
  caller references prevent simultaneous requests from colliding. Missing channel deployments never fall back to
  another namespace. Cluster rollouts now write the `.deploymentInfo` pointer used by readers and invalidation.

  The exported `paths` helpers take one options object instead of positional arguments.

- Updated dependencies [8b04ab6]
  - @granite-js/deployment-manager@2.5.3
  - @granite-js/plugin-core@2.5.3

## 2.5.2

### Patch Changes

- @granite-js/deployment-manager@2.5.2
- @granite-js/plugin-core@2.5.2

## 2.5.1

### Patch Changes

- @granite-js/deployment-manager@2.5.1
- @granite-js/plugin-core@2.5.1

## 2.5.0

### Patch Changes

- @granite-js/deployment-manager@2.5.0
- @granite-js/plugin-core@2.5.0

## 2.4.0

### Patch Changes

- @granite-js/deployment-manager@2.4.0
- @granite-js/plugin-core@2.4.0

## 2.3.2

### Patch Changes

- @granite-js/deployment-manager@2.3.2
- @granite-js/plugin-core@2.3.2

## 2.3.1

### Patch Changes

- @granite-js/deployment-manager@2.3.1
- @granite-js/plugin-core@2.3.1

## 2.3.0

### Patch Changes

- @granite-js/deployment-manager@2.3.0
- @granite-js/plugin-core@2.3.0

## 2.2.0

### Patch Changes

- @granite-js/deployment-manager@2.2.0
- @granite-js/plugin-core@2.2.0

## 2.1.1

### Patch Changes

- @granite-js/deployment-manager@2.1.1
- @granite-js/plugin-core@2.1.1

## 2.1.0

### Patch Changes

- @granite-js/deployment-manager@2.1.0
- @granite-js/plugin-core@2.1.0

## 2.0.2

### Patch Changes

- @granite-js/deployment-manager@2.0.2
- @granite-js/plugin-core@2.0.2

## 2.0.1

### Patch Changes

- @granite-js/deployment-manager@2.0.1
- @granite-js/plugin-core@2.0.1

## 2.0.0

### Patch Changes

- @granite-js/deployment-manager@2.0.0
- @granite-js/plugin-core@2.0.0

## 1.0.42

### Patch Changes

- @granite-js/deployment-manager@1.0.42
- @granite-js/plugin-core@1.0.42

## 1.0.41

### Patch Changes

- @granite-js/deployment-manager@1.0.41
- @granite-js/plugin-core@1.0.41

## 1.0.40

### Patch Changes

- @granite-js/deployment-manager@1.0.40
- @granite-js/plugin-core@1.0.40

## 1.0.39

### Patch Changes

- @granite-js/deployment-manager@1.0.39
- @granite-js/plugin-core@1.0.39

## 1.0.38

### Patch Changes

- @granite-js/deployment-manager@1.0.38
- @granite-js/plugin-core@1.0.38

## 1.0.37

### Patch Changes

- @granite-js/deployment-manager@1.0.37
- @granite-js/plugin-core@1.0.37

## 1.0.36

### Patch Changes

- @granite-js/deployment-manager@1.0.36
- @granite-js/plugin-core@1.0.36

## 1.0.35

### Patch Changes

- @granite-js/deployment-manager@1.0.35
- @granite-js/plugin-core@1.0.35

## 1.0.34

### Patch Changes

- @granite-js/deployment-manager@1.0.34
- @granite-js/plugin-core@1.0.34

## 1.0.33

### Patch Changes

- @granite-js/deployment-manager@1.0.33
- @granite-js/plugin-core@1.0.33

## 1.0.32

### Patch Changes

- @granite-js/deployment-manager@1.0.32
- @granite-js/plugin-core@1.0.32

## 1.0.31

### Patch Changes

- @granite-js/deployment-manager@1.0.31
- @granite-js/plugin-core@1.0.31

## 1.0.30

### Patch Changes

- @granite-js/deployment-manager@1.0.30
- @granite-js/plugin-core@1.0.30

## 1.0.29

### Patch Changes

- @granite-js/deployment-manager@1.0.29
- @granite-js/plugin-core@1.0.29

## 1.0.28

### Patch Changes

- @granite-js/deployment-manager@1.0.28
- @granite-js/plugin-core@1.0.28

## 1.0.27

### Patch Changes

- @granite-js/deployment-manager@1.0.27
- @granite-js/plugin-core@1.0.27

## 1.0.26

### Patch Changes

- Updated dependencies [b50c12f]
- Updated dependencies [4acc9d6]
  - @granite-js/plugin-core@1.0.26
  - @granite-js/deployment-manager@1.0.26

## 1.0.25

### Patch Changes

- d44c0b0: export withIO API
- Updated dependencies [d44c0b0]
  - @granite-js/deployment-manager@1.0.25
  - @granite-js/plugin-core@1.0.25

## 1.0.24

### Patch Changes

- d3d4e04: Await bundle uploads before updating deployment state.
- Updated dependencies [e58d561]
  - @granite-js/plugin-core@1.0.24
  - @granite-js/deployment-manager@1.0.24

## 1.0.23

### Patch Changes

- @granite-js/deployment-manager@1.0.23
- @granite-js/plugin-core@1.0.23

## 1.0.22

### Patch Changes

- @granite-js/deployment-manager@1.0.22
- @granite-js/plugin-core@1.0.22

## 1.0.21

### Patch Changes

- @granite-js/deployment-manager@1.0.21
- @granite-js/plugin-core@1.0.21

## 1.0.20

### Patch Changes

- @granite-js/deployment-manager@1.0.20
- @granite-js/plugin-core@1.0.20

## 1.0.19

### Patch Changes

- @granite-js/deployment-manager@1.0.19
- @granite-js/plugin-core@1.0.19

## 1.0.18

### Patch Changes

- @granite-js/deployment-manager@1.0.18
- @granite-js/plugin-core@1.0.18

## 1.0.17

### Patch Changes

- @granite-js/deployment-manager@1.0.17
- @granite-js/plugin-core@1.0.17

## 1.0.16

### Patch Changes

- @granite-js/deployment-manager@1.0.16
- @granite-js/plugin-core@1.0.16

## 1.0.15

### Patch Changes

- @granite-js/deployment-manager@1.0.15
- @granite-js/plugin-core@1.0.15

## 1.0.14

### Patch Changes

- @granite-js/deployment-manager@1.0.14
- @granite-js/plugin-core@1.0.14

## 1.0.13

### Patch Changes

- @granite-js/deployment-manager@1.0.13
- @granite-js/plugin-core@1.0.13

## 1.0.12

### Patch Changes

- @granite-js/deployment-manager@1.0.12
- @granite-js/plugin-core@1.0.12

## 1.0.11

### Patch Changes

- @granite-js/deployment-manager@1.0.11
- @granite-js/plugin-core@1.0.11

## 1.0.10

### Patch Changes

- @granite-js/deployment-manager@1.0.10
- @granite-js/plugin-core@1.0.10

## 1.0.9

### Patch Changes

- @granite-js/deployment-manager@1.0.9
- @granite-js/plugin-core@1.0.9

## 1.0.8

### Patch Changes

- @granite-js/deployment-manager@1.0.8
- @granite-js/plugin-core@1.0.8

## 1.0.7

### Patch Changes

- @granite-js/deployment-manager@1.0.7
- @granite-js/plugin-core@1.0.7

## 1.0.6

### Patch Changes

- @granite-js/deployment-manager@1.0.6
- @granite-js/plugin-core@1.0.6

## 1.0.5

### Patch Changes

- @granite-js/deployment-manager@1.0.5
- @granite-js/plugin-core@1.0.5

## 1.0.4

### Patch Changes

- @granite-js/deployment-manager@1.0.4
- @granite-js/plugin-core@1.0.4

## 1.0.3

### Patch Changes

- 7694332: chore: RN 0.84-rc.5 -> RN 0.84 bump up
- Updated dependencies [7694332]
  - @granite-js/deployment-manager@1.0.3
  - @granite-js/plugin-core@1.0.3

## 1.0.2

### Patch Changes

- b325495: fix(react-native-svg): buffer deps
- Updated dependencies [d4beb3b]
- Updated dependencies [b325495]
  - @granite-js/plugin-core@1.0.2
  - @granite-js/deployment-manager@1.0.2

## 1.0.1

### Patch Changes

- Updated dependencies [ee26531]
  - @granite-js/deployment-manager@1.0.1
  - @granite-js/plugin-core@1.0.1

## 1.0.0

### Major Changes

- 260daab: feat: introduce support react native 0.84

### Patch Changes

- Updated dependencies [260daab]
  - @granite-js/deployment-manager@1.0.0
  - @granite-js/plugin-core@1.0.0
