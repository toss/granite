...
export class PageResourceLoader extends Common.ObjectWrapper.ObjectWrapper {
    ...
    async dispatchLoad(url, initiator) {
        ...
        if (eligibleForLoadFromTarget) {
            try {
                if (initiator.target) {
                    ...
                    Host.rnPerfMetrics.developerResourceLoadingStarted(parsedURL, 0 /* Host.UserMetrics.DeveloperResourceLoaded.LoadThroughPageViaTarget */);
                    const result = await this.loadFromTarget(initiator.target, initiator.frameId, url);
                    Host.rnPerfMetrics.developerResourceLoadingFinished(parsedURL, 0 /* Host.UserMetrics.DeveloperResourceLoaded.LoadThroughPageViaTarget */, result);
                    return result;
                }
                ...
                if (frame) {
                    ...
                    Host.rnPerfMetrics.developerResourceLoadingStarted(parsedURL, 1 /* Host.UserMetrics.DeveloperResourceLoaded.LoadThroughPageViaFrame */);
                ...
                Host.rnPerfMetrics.developerResourceLoadingFinished(parsedURL, 2 /* Host.UserMetrics.DeveloperResourceLoaded.LoadThroughPageFailure */, { success: false, errorDescription: { message: failureReason } });
            }
            Host.userMetrics.developerResourceLoaded(3 /* Host.UserMetrics.DeveloperResourceLoaded.LoadThroughPageFallback */);
            Host.rnPerfMetrics.developerResourceLoadingStarted(parsedURL, 3 /* Host.UserMetrics.DeveloperResourceLoaded.LoadThroughPageFallback */);
        }
        else {
        ...
        Host.rnPerfMetrics.developerResourceLoadingFinished(parsedURL, 4 /* Host.UserMetrics.DeveloperResourceLoaded.FallbackAfterFailure */, result);
        return result;
    }
