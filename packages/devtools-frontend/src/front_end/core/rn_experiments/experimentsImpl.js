 ...
 */
export function setIsReactNativeEntryPoint(value) {
    if (state.didInitializeExperiments) {
        throw new Error('setIsReactNativeEntryPoint must be called before constructing MainImpl');
    }
...
function normalizePredicate(pred, defaultValue) {
...
class RNExperimentsSupport {
    #experiments = new Map();
    #defaultEnabledCoreExperiments = new Set();
    register(spec) {
        if (state.didInitializeExperiments) {
            throw new Error('Experiments must be registered before constructing MainImpl');
        }
        const { name } = spec;
     ...
     */
    enableExperimentsByDefault(names) {
        if (state.didInitializeExperiments) {
            throw new Error('Experiments must be configured before constructing MainImpl');
        }
        for (const name of names) {
            if (Object.prototype.hasOwnProperty.call(RNExperimentName, name)) {
                ...
                this.#defaultEnabledCoreExperiments.add(name);
            }
        }
    }
    copyInto(other, titlePrefix = '') {
        for (const [name, spec] of this.#experiments) {
            other.register(name, titlePrefix + spec.title, spec.unstable, spec.docLink, spec.feedbackLink);
...
//# sourceMappingURL=experimentsImpl.js.map