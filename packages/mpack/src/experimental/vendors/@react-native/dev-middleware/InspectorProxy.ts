...
export class InspectorProxy {
  ...
  /**
   * Build list of page descriptions from all devices
   */
  getPageDescriptions() {
    const result: any[] = [];
    for (const [deviceId, device] of this._devices.entries()) {
      // Simple page description for now
      result.push({
        id: deviceId,
  ...
  private createDeviceWebSocketServer(onConnected: (socket: ws.WebSocket) => void): ws.WebSocketServer {
    ...
    wss.on('connection', async (socket, request) => {
      try {
        ...
        const newDevice = new Device(deviceId, deviceName, appName, socket, this._projectRoot, null);
