...
The Android path is the real-device QA target. The iOS project includes the
equivalent `UIViewController`-owned host integration and contract coverage,
but has not completed the same device QA scenario.
